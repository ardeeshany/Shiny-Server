...
##
  ...
  s1 <-ggplot(slope1,aes(x = reorder(names,sl),y = sl))+
    geom_bar(stat="identity",aes(fill = factor(clr, labels = c("پسرفت","پیشرفت"))),color=slope1$ann[dim(slope1)[1]:1])+
    labs(fill="")+
...
s<-ggplot(slope,aes(x=reorder(names,1:4), y = sl))+
  geom_bar(stat="identity",aes(fill = factor(clr, labels = c("پسرفت","پیشرفت"))),color="black")+
  labs(fill="")+  # legend title
  geom_text(data=slope,aes(x = reorder(names,1:4),y = sl,label=round(sl,3)),vjust=0)+
  ylab("شیب پیشرفت خطی") + xlab("")+
  scale_x_discrete(labels= names_ch[4:1],position = "bottom")+
  theme(axis.text.x=element_text(face = "bold.italic", color = "blue", size = 8),
        axis.text.y=element_text(colour="blue", size=10, face="bold"))+
  coord_flip()  
